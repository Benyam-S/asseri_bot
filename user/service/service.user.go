...
func NewUserService(userRepository user.IUserRepository, jobRepository job.IJobRepository,
	jobApplicationRepository jobapplication.IJobApplicationRepository,
	commonRepository common.ICommonRepository) user.IService {
	return &Service{userRepo: userRepository,
		jobRepo: jobRepository, jobApplicationRepo: jobApplicationRepository, commonRepo: commonRepository}
}
...
func (service *Service) AllUsersWithPagination(category string, pageNum int64) ([]*entity.User, int64) {
	...
	if category != entity.UserCategoryasseri && category != entity.UserCategoryAgent &&
...
func (service *Service) TotalUsers(category string) int64 {
	...
	if category != entity.UserCategoryAgent && category != entity.UserCategoryasseri &&
		category != entity.UserCategoryJobSeeker {
		category = entity.UserCategoryAny
	}
...
func (service *Service) DeleteUser(userID string) (*entity.User, error) {
	...
	jobs := service.jobRepo.FindMultiple(userID)
	for _, job := range jobs {
		if job.Status == entity.JobStatusOpened {
			service.jobRepo.UpdateValue(job, "status", entity.JobStatusClosed)
		}
	}
